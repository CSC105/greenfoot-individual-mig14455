import greenfoot.*; 
public class YellowFlamingo extends Actor
{
    private int ySpeed;
    private int img;
    ...
    public YellowFlamingo() 
    {
        ...
        for(int i=0 ; i<36 ; i++){
            rightImage[i] = new GreenfootImage("Yellow/Flamingo"+(i+1)+".png");
            leftImage[i] = new GreenfootImage("Yellow/Flamingo"+(i+1)+".png");
    ...
    public void act() {
        ...
        if(spaceDown != Greenfoot.isKeyDown("0")){
            fireRate--;
            if(fireRate == 0){
                shooting();
                fireRate = 10;
        ...
        if(this.isTouching(LeftFishForPink.class)){
            removeTouching(LeftFishForPink.class);
            setLocation(getX()-10, getY());
            lives--;
            if(lives == 0){
                Pinkwin result = new Pinkwin();
                Greenfoot.setWorld(result);
                Greenfoot.playSound("over.wav");
            }
        }
        if(this.isTouching(RightFishForPink.class)){
            removeTouching(RightFishForPink.class);
            setLocation(getX()+10, getY());
            lives--;
            if(lives == 0){
                Pinkwin result = new Pinkwin();
    ...
    public void jumpOnBlock(){
        ...
        for(int i = 0; i < 30; i++) {
            Actor Blocks = getOneObjectAtOffset(i,-30, Blocks.class);
    ...
    public void shooting(){
        ...
        if(direction == 1){ //face to right
            getWorld().addObject(new RightFishForYellow(),getX()+20,getY()-15);

        }
        if (direction == -1) { //face to left
            getWorld().addObject(new LeftFishForYellow(),getX()-20,getY()-15);
